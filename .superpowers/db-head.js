...
function switchView(view) {
  ...
  if (view === "viz") { setTimeout(function(){ window.initViz(); }, 100); }
  ...
  renderChart("screenRealtime", "chartRealtime", {
    ...
    series: [
      { name: "烟雾", type: "line", smooth: true, data: smokeSeries, itemStyle: { color: "#2563eb" }, areaStyle: { color: "rgba(37,99,235,0.12)" },
        markLine: { silent: true, symbol: "none", label: { fontSize: 10, color: "#ef4444", formatter: "HIGH {c}" },
          data: [{ yAxis: 0.30, lineStyle: { color: "#ef4444", type: "dashed", width: 1 }, label: { formatter: "HIGH 0.30" } },
                 { yAxis: 0.15, lineStyle: { color: "#f59e0b", type: "dashed", width: 1 }, label: { formatter: "MED 0.15" } }] }
      },
      { name: "温度", type: "line", smooth: true, data: tempSeries, itemStyle: { color: "#f97316" }, areaStyle: { color: "rgba(249,115,22,0.08)" },
        markLine: { silent: true, symbol: "none", label: { fontSize: 10, color: "#ef4444" },
          data: [{ yAxis: 65, lineStyle: { color: "#ef4444", type: "dashed", width: 1 }, label: { formatter: "HIGH 65°C" } }] }
      },
    ],
...
var _deviceThresholds = [];
async function showDeviceThresholdModal(deviceId, deviceCode) {
  try { var data = await apiRequest("/thresholds?page=1&pageSize=200"); _deviceThresholds = (data&&data.records)||[]; }
  catch(e) { _deviceThresholds = []; }
  var devThr = _deviceThresholds.filter(function(t) { return String(t.deviceId) === String(deviceId); });
  ...
  var modal = el("detailModal"); var title = el("detailModalTitle"); var body = el("detailModalBody");
  if (!modal||!title||!body) return;
  title.textContent = '阈值配置: ' + (deviceCode||deviceId);
  body.innerHTML = '<div style="padding:10px;">'+
    '<div class="form-group"><label>烟雾阈值 HIGH (mg/m³)</label><input id="thrSmokeHigh" value="'+(sH?sH.thresholdMax:'0.30')+'" style="width:100%;padding:8px;border:1px solid #d1d5db;border-radius:6px;margin:4px 0;"></div>'+
    '<div class="form-group"><label>烟雾阈值 MEDIUM (mg/m³)</label><input id="thrSmokeMed" value="'+(sM?sM.thresholdMax:'0.15')+'" style="width:100%;padding:8px;border:1px solid #d1d5db;border-radius:6px;margin:4px 0;"></div>'+
    '<div class="form-group"><label>温度阈值 HIGH (°C)</label><input id="thrTempHigh" value="'+(tH?tH.thresholdMax:'65')+'" style="width:100%;padding:8px;border:1px solid #d1d5db;border-radius:6px;margin:4px 0;"></div>'+
    '<button class="btn btn-main" style="width:100%;margin-top:10px;" onclick="saveDeviceThresholds(\''+deviceId+'\')">保存阈值</button>'+
    '</div>';
  modal.classList.remove("hidden");
}
async function saveDeviceThresholds(deviceId) {
  var sH = parseFloat(el("thrSmokeHigh").value)||0.30;
  var sM = parseFloat(el("thrSmokeMed").value)||0.15;
  var tH = parseFloat(el("thrTempHigh").value)||65;
  // delete old thresholds for this device
  var old = _deviceThresholds.filter(function(t) { return String(t.deviceId) === String(deviceId); });
...
function renderDevicesTable() {
  ...
  body.querySelectorAll("button[data-device-threshold]").forEach((button) => button.addEventListener("click", () => {
    showDeviceThresholdModal(button.dataset.id, button.dataset.code);
  }));
